...
class DateTimeTool {
  static const definition = ToolDefinition(
    ...
    parameters: {
      ...
      'properties': {
        'format': {
          'type': 'string',
          'description': 'Optional: "full" for complete info, "date" for date only, "time" for time only, "unix" for unix timestamp. Default: "full"',
        },
      },
    },
  );
  ...
  static Future<ToolResult> execute(String toolCallId, Map<String, dynamic> args) async {
    ...
    switch (format) {
      case 'date':
        output = 'Date: ${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')}';
      case 'time':
        output = 'Time: ${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}:${now.second.toString().padLeft(2, '0')}';
      case 'unix':
    ...
    return ToolResult(toolCallId: toolCallId, name: definition.name, output: output);
  }
}
