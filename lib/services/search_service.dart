...
class SearchService {
  ...
  static Future<List<SearchResult>> search(String query,
    ...
    try {
      ...
      final linkRe = RegExp(
          ...
          dotAll: true);
      final snippetRe = RegExp(r'<a[^>]+class="result__snippet"[^>]*>(.*?)</a>',
          dotAll: true);
      ...
      for (int i = 0; i < links.length && results.length < maxResults; i++) {
        ...
        final snippet =
            i < snippets.length ? _stripTags(snippets[i].group(1) ?? '') : '';
        if (url.isNotEmpty && title.isNotEmpty) {
          final actualUrl = _extractUrl(url);
          results.add(
              SearchResult(title: title, url: actualUrl, snippet: snippet));
        }
      }
      return results;
    } catch (e) {
  ...
  static Future<String> fetchPageContent(String url,
      {int maxChars = 3000}) async {
    try {
